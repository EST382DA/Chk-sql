...
mycon = sqlctr.connect(host='localhost', user='root', password='EST-382DA',database='pathsala')
...
cursor.execute("create table if not exists books(SN int(50) primary key,Book_Name varchar(600), Quantity_Available int(100),Price_Per_Day decimal(55,2))")
cursor.execute("create table if not exists BORROWER(SN int(50),borrowers_name varchar(400),book_lent varchar(200),contact_date date,contact_no varchar(100))")
...
def detail_borrower(name,contact):
    tup=('SN','borrowers_name','book_lent','contact_date','contact_no')
...
def borrowers():
    ...
    if dec=='1':
        ...
        name = input('\nenter borrower name-')
        contact = input('enter borrower contact no.-')
...
def tfine():
    ...
    for i in data:
        book_name=i[0]
        st2 = 'select contact_date from borrower where borrowers_name="{}" and book_lent="{}"'.format(name,book_name)
        command(st2)
        data1=cursor.fetchall()
        for date in data1:
            date_taken=date[0]
            date_return = str(input('\nEnter returning contact_date for book "{}" (YYYY-MM-DD) , Press ENTER to skip-'.format(book_name)))
            while date_return!='':
